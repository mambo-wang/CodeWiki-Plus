...
class _Pattern:
    def __init__(self, pattern: re.Pattern, broker: str, role: RouteRole,
                 topic_group: int = 1):
...
_KAFKA_PRODUCER = _Pattern(
    ...
    broker="kafka", role=RouteRole.CLIENT, topic_group=1,
)
...
_KAFKA_CONSUMER = _Pattern(
    ...
    broker="kafka", role=RouteRole.SERVER, topic_group=1,
)
...
_RABBIT_PRODUCER = _Pattern(
    ...
    broker="rabbitmq", role=RouteRole.CLIENT, topic_group=1,
)
...
_RABBIT_CONSUMER = _Pattern(
    ...
    broker="rabbitmq", role=RouteRole.SERVER, topic_group=1,
)
...
_ROCKET_PRODUCER = _Pattern(
    ...
    broker="rocketmq", role=RouteRole.CLIENT, topic_group=1,
)
...
_ROCKET_CONSUMER = _Pattern(
    ...
    broker="rocketmq", role=RouteRole.SERVER, topic_group=1,
)
...
_CELERY_PRODUCER = _Pattern(
    ...
    broker="celery", role=RouteRole.CLIENT, topic_group=1,
)
...
_CELERY_CONSUMER = _Pattern(
    ...
    broker="celery", role=RouteRole.SERVER, topic_group=1,
)
...
ALL_PATTERNS = [
    ...
    _RABBIT_PRODUCER, _RABBIT_CONSUMER,
    _ROCKET_PRODUCER, _ROCKET_CONSUMER,
    _CELERY_PRODUCER, _CELERY_CONSUMER,
    _PY_KAFKA_PRODUCER,
    _GO_KAFKA_PRODUCER,
]
...
def extract_mq_routes(file_path: str, content: str, repo_name: str) -> List[RouteNode]:
    ...
    for pat in ALL_PATTERNS:
        for m in pat.pattern.finditer(content):
            topic = m.group(pat.topic_group) if pat.topic_group <= len(m.groups()) and m.group(pat.topic_group) else ""
            if not topic:
...
def _find_enclosing_function(content: str, pos: int) -> Optional[str]:
    ...
    patterns = [
        ...
        re.compile(r'(?:public|private|protected|static|final|synchronized|abstract|\s)+\s+\S+\s+(\w+)\s*\('),
