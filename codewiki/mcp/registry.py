...
_register(
    Tool(
        name="stage_task_memories",
        description=(
            "Stage distilled task memories in the task's pending area "
            "(repowiki/tasks/<task_id>/pending-memories.json) awaiting user "
            "confirmation. Nothing lands in memories.md yet — call "
            "confirm_task_memories to persist or reject_task_memories to drop. "
            "Duplicates already staged are skipped. Used automatically by "
            "distill_conversation when a raw conversation carries a task_id."
        ),
        inputSchema={
            "type": "object",
            "properties": {
                ...
                "memories": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Memory texts (markdown) to stage.",
                },
                "source_raw": {
                    "type": "string",
                    "description": "Optional source raw file relpath for traceability.",
                },
                "session_id": {"type": "string", "description": "Optional active session id."},
                "output_dir": {"type": "string", "description": "Wiki output directory."},
                "repo_path": {"type": "string", "description": "Repository path."},
    ...
    handler_path="codewiki.mcp.tools.task_manager:handle_stage_task_memories",
    mode="thread",
)
...
_register(
    Tool(
        ...
        description=(
            "List a task's pending (staged, unconfirmed) memories — the entries "
            "produced by distill_conversation that await user confirmation before "
            "they land in memories.md."
        ),
        inputSchema={
            "type": "object",
            "properties": {
    ...
    handler_path="codewiki.mcp.tools.task_manager:handle_list_pending_memories",
    mode="thread",
)

_register(
    Tool(
        name="confirm_task_memories",
        description=(
    ...
    handler_path="codewiki.mcp.tools.task_manager:handle_confirm_task_memories",
    mode="thread",
)

_register(
    Tool(
        name="reject_task_memories",
        description=(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "reason": {
                    "type": "string",
                    "description": "Optional informational reason (not persisted).",
                },
                "session_id": {"type": "string", "description": "Optional active session id."},
                "output_dir": {"type": "string", "description": "Wiki output directory."},
                "repo_path": {"type": "string", "description": "Repository path."},
