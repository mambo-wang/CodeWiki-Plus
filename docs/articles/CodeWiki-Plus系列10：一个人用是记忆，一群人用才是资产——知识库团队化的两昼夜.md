...
| 动作 | 时机 | 关键约束 |
|------|------|----------|
| **拉取校验**（只提醒，不动手） | 每进程每仓库一次 | 只做只读的远端查询，只告诉你"远端新了 N 个提交，建议先同步"，不执行任何同步 |
| **快进拉取**（只进不退的更新） | 会话开始时 | 每进程每仓库一次；只做"快进"，永远不自动合并、不自动变基；不预设"工作区脏就不能拉"——交给 git 自己的覆盖保护裁决：更新绕开你的改动就正常快进，会覆盖你的改动就整体拒绝、一个文件都不动 |
| **自动暂存**（默认开启） | 每次知识写入后 | 只 `git add`，永不提交、永不推送；`.gitignore` 就是策略载体——被忽略的路径天然加不进去，锁文件等残留额外硬跳过 |
| **写后推送** | 一批写入完成后（关会话、批量入库、对话采集、蒸馏提交等天然边界） | 默认关闭；提交用路径限定（pathspec）只圈知识子树，你在子树外暂存的内容原样留在暂存区，永远不会被卷进知识提交 |

自动暂存值得多说一句：它的钩子挂在所有写入的唯一收口上（原子写文件那一个函数），一处挂钩、全量覆盖。效果是知识文件写完就出现在 `git status` 的暂存区里，操作员自己的提交随时可做，不用再手动 `git add`。而"哪些文件该入库"的策略不写在代码里，就写在 `.gitignore` 里——`git add` 不带 `-f` 会拒绝被忽略的路径，忽略清单本身就是策略。

贯穿四者的一条不变量，值得单独拎出来：**数据不丢，晚点没关系。** 任何失败——没装 git、断网、凭证过期、推送撞车——都降级成"报告一句、本地状态原样保留"：推送失败时本地提交留着，下次成功推送自动搭载；绝不强推、绝不回滚、绝不删改动。

### 一条被推翻的红线

"不碰你已经暂存的内容"最初是这么实现的：发现暂存区里有任何非工具的改动，直接放弃本次推送并告诉你原因。听起来很谨慎，对吧？真实仓库里它变成了高频的静默哑火——用户随手 `git add` 一个文件是再正常不过的操作，于是"我明明开了自动推送，为什么没推？"成了排名第一的抱怨。**守卫挡住的恰恰是它要服务的人。**

修法不是放松警惕，而是换武器：把"整仓提交"改成"路径限定提交"——`git commit -- <repowiki>/` 只提交知识子树内的内容。你在子树外暂存的改动原样留在暂存区，一个字节都不会被卷进知识提交；子树内你暂存的内容本来就是要入库的知识，顺路带走。守卫从"看到别人暂存就罢工"变成了"各管各的路径"，边界反而更清晰。

没退让的边界只剩三条，而且都是同一句话的不同侧面——**工具绝不替你处置你的东西**：

- **不碰你子树外的暂存内容。** 路径限定提交保证你的暂存区永远只属于你；
- **不强制推送、不回滚、不删改动。** 失败就保留现场，交回人工；
- **不替你裁决版本纠纷。** 真岔了就只报个案——"本地领先几笔、落后几笔，建议先同步"，一个字符都不动。

一句话版本：**红线从"我不敢碰"，细化成了"我不替你做决定"；而其中一条具体做法，被真实使用推翻后换成了更精确的做法——红线的精神没变，实现更诚实了。**
