...
*本文基于 CodeWiki-Plus 当前实现撰写，相关模块：`codewiki/mcp/tools/distill_conversation.py`（L1 纪律与两段式去重、L0 归档）、`note_consolidation.py`（L2 场景聚合）、`doctrine.py`（L3 Doctrine）、`aggregation_state.py`（计数器与提醒）、`knowledge_loop.py`（检索溯源露出）。设计文档见 `docs/团队记忆融合-L2场景聚合与L3-Doctrine设计方案.md`，TAM 调研见 `docs/TencentDB-Agent-Memory-记忆提取机制分析.md`。*
