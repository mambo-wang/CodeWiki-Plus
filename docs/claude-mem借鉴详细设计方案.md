...
> 日期：2026-09-02 · Rev.2（拷问评审定稿，15 项裁决）· 状态：**评审定稿** · 预计工作量：**3 人日**（P0-1 → P0-2 含 P1-4 → P0-3 收尾）
> 范围约束：
> - **不新增 MCP 工具**（沿用 `query_wiki` 现有参数面扩展）；
> - **不改评审闸门哲学**（`confirm_note` / `reject_note` 保留，ADR-0002 不变）；
> - **不改 Markdown 存储路线**（不迁 SQLite，结构化字段一律可选、不破坏 frontmatter 往返不变量）；
...
| ~~P1-2~~ | 笔记 `files` 结构化字段 | 提升 P0-2 匹配精度 | 0.5 人日 | **v1.5 可选**，见 §2.6 |
| ~~P1-3~~ | 折叠视图（smart_outline 等价物） | 填补索引与全文之间的空档 | 3–4 人日 | **单独排期**，见 §5 |
...
| Doctrine / 场景聚合 | P1-5（保留 filter 支持 rebuild）未纳入本次范围，见调研报告 §5.2 |
