...
def main():
    ...
    r = json.loads(asyncio.run(handle_write_doc_file({
        "session_id": sid, "filename": "okf_new.md",
    ...
    r = json.loads(asyncio.run(handle_write_doc_file({
        "session_id": sid, "filename": "okf_agent.md",
        "content": "---\ntitle: 自定义标题\ncustom_key: keep-me\n---\n# 代理文档\n\n正文。\n",
    }, store)))
    ...
    r = json.loads(asyncio.run(handle_write_doc_file({
        "session_id": sid, "filename": "okf_typed.md",
        "content": "---\ntype: Concept\ntitle: 已有类型\n---\n# 已带类型\n\n正文。\n",
    }, store)))
    doc3 = Path(r.get("path", ""))
    if doc3.exists():
        fm3 = read_fm(doc3)
        check("write_doc_file", "已有type不重复注入", fm3.count("type:") == 1 and "Concept" in fm3, fm3[:300])

    # ================================================================
    print("\n[2] edit_doc_file — frontmatter 存在时的编辑")
    r = json.loads(asyncio.run(handle_edit_doc_file({
        "session_id": sid, "filename": "okf_new.md",
        "command": "str_replace", "old_string": "# 新文档", "new_string": "# 新文档V2",
    }, store)))
    ...
    r = json.loads(handle_ingest_note({
        "session_id": sid, "note_type": "decision",
        "title": "OKF回归测试决策", "content": "这是OKF回归测试的决策笔记内容。",
    }, store))
    ...
    r = json.loads(handle_ingest_note({
        "session_id": sid, "note_type": "lesson",
        "title": "显式stable笔记", "content": "这条笔记显式指定stable状态。",
        "status": "stable",
    }, store))
    check("ingest_note", "显式status=stable接受", r.get("note_status") == "stable", str(r)[:200])

    r = json.loads(handle_ingest_note({
        "session_id": sid, "note_type": "pitfall",
        "title": "旧词汇candidate笔记", "content": "这条笔记用旧词汇candidate。",
        "status": "candidate",
    }, store))
    note3 = Path(r.get("note_path", ""))
    check("ingest_note", "legacy candidate归一化为draft", r.get("note_status") == "draft", str(r)[:200])

    # ================================================================
    print("\n[4] confirm_note — stable升级与verified事件")
    r = json.loads(handle_confirm_note({
        "session_id": sid, "note_file": note1.name, "by": "human:tester",
    }, store))
    check("confirm_note", "升级为stable", r.get("status") == "stable", str(r)[:200])
    fm = read_fm(note1)
    check("confirm_note", "记录verified事件(human:tester)",
          "verified:" in fm and "human:tester" in fm, fm[:500])
    import re as _re
    ...
    r = json.loads(handle_ingest_note({
        "session_id": sid, "note_type": "general",
        "title": "待否决笔记", "content": "这条笔记将被否决。",
    }, store))
    note4 = Path(r.get("note_path", ""))
    r = json.loads(handle_reject_note({
        "session_id": sid, "note_file": note4.name, "reason": "回归测试否决",
    }, store))
    ...
    r = json.loads(handle_query_wiki({
        "session_id": sid, "query": "legacyquery 旧确认笔记",
    }, store))
    ...
    if legacy_hits:
        check("query_wiki", "legacy confirmed无[unconfirmed]前缀",
              "[unconfirmed]" not in legacy_hits[0].get("title", ""), str(legacy_hits[0])[:200])
    ...
    if conf_hits:
        check("query_wiki", "trust_tier=human-reviewed",
              conf_hits[0].get("trust_tier") == "human-reviewed", str(conf_hits[0])[:300])
    ...
    r = json.loads(handle_ingest_source({
        "session_id": sid, "source_ref": str(src_file), "name": "ext-spec",
        "source_type": "md", "description": "外部规范",
    ...
    if stored.exists():
        fm = read_fm(stored)
        check("ingest_source", "md源注入type/status/generated",
              "type:" in fm and "status:" in fm and "generated:" in fm, fm[:400])
    fm1 = read_fm(doc1)
    check("ingest_source", "相关页面注入sources条目",
          "sources:" in fm1 and "ext-spec" in fm1, fm1[:600])

    r = json.loads(handle_ingest_source({
        "session_id": sid, "source_ref": str(src_file), "name": "ext-spec-dup",
    }, store))
    check("ingest_source", "重复内容检测", r.get("status") == "duplicate", str(r)[:200])

    # ================================================================
    print("\n[8] retract_source — dry_run与引用清理")
    r = json.loads(handle_retract_source({
        "session_id": sid, "name": "ext-spec", "mode": "remove_refs", "dry_run": True,
    }, store))
    ...
    r = json.loads(handle_retract_source({
        "session_id": sid, "name": "ext-spec", "mode": "remove_refs",
    }, store))
    ...
    r = json.loads(handle_batch_ingest({
        "session_id": sid,
        "items": [
            {"kind": "note", "note_type": "general", "title": "批量笔记一",
             "content": "批量导入的笔记内容。"},
    ...
    items = r.get("results", [])
    check("batch_ingest", "2项全部ok",
          len(items) == 2 and all(i.get("status") == "ok" for i in items), str(items)[:300])
    ...
    check("lint_wiki", "检出无frontmatter(error)",
          any(i.get("severity") == "error" and "bad_nofm" in i.get("file", "") for i in okf_issues),
          str(kinds)[:300])
    check("lint_wiki", "检出legacy status(warning)",
          any("bad_legacy" in i.get("file", "") and "Legacy status" in i.get("message", "") for i in okf_issues),
          str(kinds)[:300])
    check("lint_wiki", "检出过期stale_after",
          any("bad_expired" in i.get("file", "") for i in okf_issues), str(kinds)[:300])
    check("lint_wiki", "检出坏verified",
          any("bad_verified" in i.get("file", "") for i in okf_issues), str(kinds)[:300])
    ...
    check("get_prompt", "含OKF v0.2合规段", "OKF (Open Knowledge Format) v0.2" in prompt_text, prompt_text[:200])
    ...
    r = json.loads(handle_get_prompt(
        {"prompt_type": "system_leaf", "output_dir": str(output_dir)}, store))
    prompt_text2 = r.get("content", "") or json.dumps(r, ensure_ascii=False)
    check("get_prompt", "仅output_dir也注入OKF段",
          "OKF (Open Knowledge Format) v0.2" in prompt_text2, prompt_text2[:200])
    ...
    if idx.exists():
        ...
        check("index", "frontmatter含okf_version", "okf_version" in read_fm(idx), read_fm(idx)[:150])
    ...
    check("schema_generator", "okf_version默认0.2", conv.get("okf_version") == "0.2", str(conv)[:200])
    check("schema_generator", "default_stale_days默认90", conv.get("default_stale_days") == 90, str(conv)[:200])
