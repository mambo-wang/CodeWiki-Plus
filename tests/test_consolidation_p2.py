...
# Helpers
# --------------------------------------------------------------------------- #
def _set_thresholds(repo: str, cons_t: int = 3, hint_interval: int = 2, max_scenes: int = 15):
...
def _ingest(
    repo: str, title: str, note_type: str = "decision", content: str = "## Background\nbody"
) -> str:
    store = SessionStore()
    r = json.loads(
        handle_ingest_note(
...
def _write_scenario(
    repo: str, name: str, body: str = "## Work context\nx", with_provenance: bool = True
) -> str:
...
def test_submit_records_provenance_and_resets_counter(tmp_path):
    ...
    scen = _write_scenario(
        repo, "redis-运维方法", body="## Core SOP\ncheck pool", with_provenance=False
    )
    resp = _consolidate(
        repo,
        {
...
def test_submit_capacity_exceeded_blocks_reset(tmp_path):
    ...
    resp = _consolidate(
        repo,
        {
            "mode": "submit",
            "report": {
                "scenarios": [{"file": f, "action": "updated", "source_notes": []} for f in files]
            },
        },
    )
