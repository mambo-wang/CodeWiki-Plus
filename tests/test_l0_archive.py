...
def test_drop_raw_argument_deletes(tmp_path):
    ...
    _data, by_cid = _submit(repo, {cid: {"notes": [{
        "title": "privacy note", "note_type": "general", "content": "x",
    }]}}, drop_raw=True)
    r = by_cid[f"conv-{cid}"]
...
def test_drop_raw_frontmatter_deletes(tmp_path):
    ...
    _data, by_cid = _submit(repo, {cid: {"notes": [{
        "title": "privacy fm note", "note_type": "general", "content": "x",
    }]}})
    r = by_cid[f"conv-{cid}"]
    assert r["deleted_raw"] is True
    assert r["archived_raw"] is None
...
def test_source_ref_repointed_to_archive(tmp_path):
    ...
    _write_raw(repo, cid)
    _data, by_cid = _submit(repo, {cid: {"notes": [{
        "title": "traceable note", "note_type": "decision",
        "content": "## Background\ntrace me",
    }]}})
    r = by_cid[f"conv-{cid}"]
...
def test_source_ref_repointed_across_conflict_rounds(tmp_path):
    ...
    r0 = json.loads(handle_ingest_note({
        "output_dir": f"{repo}/repowiki",
        "title": "alpha beta gamma delta epsilon",
        "note_type": "pitfall", "content": "existing", "status": "draft",
    }, store))
    handle_confirm_note({
        "output_dir": f"{repo}/repowiki",
    ...
    notes_json = {
        "notes": [
            {"title": "alpha beta gamma zeta eta", "note_type": "lesson",
             "content": "weak duplicate candidate"},
            {"title": "completely unrelated knowledge", "note_type": "decision",
    ...
    resolve_json = {
        "notes": [
            {"title": "alpha beta gamma zeta eta", "note_type": "lesson",
             "content": "weak duplicate candidate", "dedup_action": "store"},
        ]
    }
    ...
    refs = {_note_source_ref(repo, n) for n in _notes(repo)
            if n != Path(r0["note_path"]).name}
...
def test_archived_conversations_not_searchable(tmp_path):
    ...
    _data, by_cid = _submit(repo, {cid: {"notes": [{
        "title": "noindex note", "note_type": "general",
...
def test_query_wiki_surfaces_source_ref(tmp_path):
    ...
    _data, by_cid = _submit(repo, {cid: {"notes": [{
        "title": "缓存击穿防护决策", "note_type": "decision",
        "content": "## Background\n缓存击穿需要互斥锁",
    }]}})
    assert by_cid[f"conv-{cid}"]["archived_raw"]
    ...
    note_hits = [r for r in resp.get("results", [])
                 if r.get("file", "").startswith("notes/")]
