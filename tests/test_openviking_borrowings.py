...
def test_v3_merge_notes_field_strategies(tmp_path):
    ...
    a = _mk_note(tmp_path, "a.md", "旧标题", "旧结论正文", date="2026-01-01", related=["mod_a"])
    b = _mk_note(tmp_path, "b.md", "新标题", "新结论正文", date="2026-06-01", related=["mod_b"])
...
def test_v2_budget_config_and_defaults():
    ...
    custom = {
        "conventions": {
            "injection_budget": {"search_result_chars": 50, "agents_md_module_lines": 2}
        }
    }
...
def test_v2_snippet_degradation(tmp_path):
    ...
    results = [
        {"file": "wiki/modules/A.md", "snippet": "x" * 400, "relevance_score": 1.5},
        {"file": "wiki/modules/B.md", "snippet": "y" * 400, "relevance_score": 1.2},
    ]
...
def test_v2_degraded_line_uses_description(tmp_path):
    ...
    results = [{"file": "wiki/modules/A.md", "snippet": "s" * 2000, "relevance_score": 2.0}]
