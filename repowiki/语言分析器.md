...
语言分析器模块是 CodeWiki-CN 依赖分析引擎的多语言 AST 解析层，为每种支持的编程语言提供专门的语法分析器。该模块通过两种技术路线实现代码组件提取：Python 使用内置 `ast` 模块，其余 8 种语言（Java、JavaScript、TypeScript、C、C++、C#、PHP、Kotlin）使用 Tree-sitter 增量解析框架。所有分析器输出统一的 `Node` 和 `CallRelationship` 数据模型，供上层 [分析服务](分析服务.md) 统一消费。
