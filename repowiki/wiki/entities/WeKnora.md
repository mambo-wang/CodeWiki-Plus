...
WeKnora（维娜拉）是腾讯开源的、基于大语言模型（LLM）的知识管理框架，专为企业级文档理解、语义检索与智能推理场景打造 。项目基于 MIT 协议发布，可自由使用、修改与分发，但需保留原始版权声明 。
...
- 多源数据接入：飞书 / Notion / 语雀 / RSS 自动同步，覆盖 PDF、Word、图片、Excel 等十余种文档格式 
- 企业级多空间 [空间RBAC](../concepts/空间RBAC.md)：四级角色矩阵 + 资源归属 + 空间审计日志 
- 网站嵌入 Widget、权限范围 API Key 与 Principal 模型、每空间多实例存储后端 
- 全流程模块化：大模型、向量数据库、存储等组件均可灵活替换，支持本地与私有云部署，数据完全自主可控 
- 可观测性：集成 [Langfuse](Langfuse.md) 追踪 Agent 运行、Token 消耗与任务流水线 
- [文档知识图谱](../concepts/文档知识图谱.md)：将文档转化为知识图谱，为索引和检索提供结构化支撑 
...
依赖 Docker 与 Docker Compose；`git clone` 后配置 `.env`，`docker compose up -d` 启动核心服务，访问 http://localhost 使用 。支持本地 / Docker / Kubernetes (Helm) 部署与私有化离线部署 。界面形态包括 Web UI、RESTful API、`weknora` 命令行、Chrome Extension、网站嵌入 Widget 与微信小程序 。

## 生态与集成

配套 Chrome 插件（网页内容一键采集） 、微信小程序 、[ClawHub Skill](ClawHubSkill.md)  与配套 MCP 服务器 。WeKnora 同时是 [微信对话开放平台](微信对话开放平台.md) 的核心技术框架，可在公众号、小程序等微信场景中提供问答服务 。IM 集成覆盖企业微信 / 飞书 / Lark / QQBot / Slack / Telegram / 钉钉 / Mattermost / 微信 ，检索层支持 BM25、Dense、GraphRAG 等 [混合检索策略](../concepts/混合检索策略.md) 。
