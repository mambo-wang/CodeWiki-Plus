...
CodeBuddy invokes the copied script and passes the event as JSON on
**stdin**, e.g. for SessionEnd (the registered command uses a portable
per-IDE form — project-root variable like ``$CODEBUDDY_PROJECT_DIR`` or a
repo-relative path, generated by ``codewiki install-hooks`` — so the
committed ``settings.json`` works from any checkout location):
